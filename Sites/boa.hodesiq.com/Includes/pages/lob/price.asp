...
if ( hasRequestedVersion && price_video ) {
	...
	document.write(getVideo(price_video.url,"javaflash",""));
	document.write(strReplace(strReplace(strReplace(strReplace(getHTMLControl("javaflash",true,true,true,true,true,true),"<text>","Video transcript"),"<title>","Video transcript"),"<href>",ada_href),"<anchor>",price_video.ada_anchor));
	document.write('</div>');
	document.write('<div id="videothumb">');
	document.write('<a href="Javascript: vo_id();" onclick="playVideo(price_video,true,true,true,true,true,true);"><img style="margin: 0px;" src="'+price_video.thumb_on+'" alt="" border="0" align="absmiddle"></a>');
