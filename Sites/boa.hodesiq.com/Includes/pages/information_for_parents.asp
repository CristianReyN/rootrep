		<a name="skipmaincontent"></a><H1 style="margin: 0px <%=right_margin%>px 0px <%=left_margin_0%>px; padding: 0px;">Information for parents</H1>

		<table width="578" cellpadding="0" cellspacing="0" border="0" summary="">
			<tr valign="top">
				<td width="373" height="100%" valign="top" style="<%=middle_content_padding%>">
<img class="dotdiv" src="../images/clear.gif" width="100%" height="1" alt="" border="0">

<p style="margin: 0px;">
As parents, you never stop trying to create opportunities for your children - opportunities for them to succeed and prosper, to find their place in the world, and maybe even to make a lasting impression on the world and the people around them.
<br><br>
At Bank of America, the same things are important to us as a company. We work continuously to create opportunities for each and every member of our associate family. And we share your belief in the importance of doing the right thing, treating people with respect, and acting with honesty and integrity.
<br><br>
These commitments have not only made Bank of America one of the world's most admired companies, but also a place you and your children can be proud to be part of.
<br><br>
A career with Bank of America provides your child with:
</p>

<ul>
	<li>Built-in networks of success, with associates who share their ideas and common interests, through our diverse range of <a href="http://careers.bankofamerica.com/learnmore/diversity.asp" class="left2" onfocus="this.className='left2-over';" onblur="this.className='left2';" title="Affinity Groups" target="_blank">affinity groups</a></li>
	<li>Strong compensation for their skills and talents, and support for their life outside of work, through our world-class <a href="<% If page_section <> "ADA" Then %>../learnmore/<%End If%>benefitsadv.asp" class="left2" onfocus="this.className='left2-over';" onblur="this.className='left2';" title="Benefits Program">benefits program</a></li>
	<li>Professional and personal development paths through our commitment to <a href="http://careers.bankofamerica.com/staffingevents/staffingprocess.asp" class="left2" onfocus="this.className='left2-over';" onblur="this.className='left2';" title="Training And Career Advancement">training and career advancement</a></li>
	<li>A sense of involvement in and support for the communities we serve through our <a href="http://www.bankofamerica.com/foundation/" target="_blank" class="left2" onfocus="this.className='left2-over';" onblur="this.className='left2';" title="Commitment To Philanthropy">commitment to philanthropy</a></li>
	<li>Opportunities to contribute in the workplace and beyond through our unique <a href="http://www.bankofamerica.com/teambank/" target="_blank" class="left2" onfocus="this.className='left2-over';" onblur="this.className='left2';" title="Team Bank of America">Team Bank of America</a> program</li>
</ul>

				</td>
				<td width="205" height="100%" valign="top">
<!-- #include file="../collrecrnav.asp" -->
				</td>
			</tr>
		</table>