...
%>
						<tr>
							...
							<td <% If Not two_bank Then %>width="100%"<% Else %>width="50%"<% End If %> style="height: 30px; padding: 8px 0px 2px 9px;" valign="top"><div style="margin: 0px;"><label class="bac" for="q-<%=questions(q).Item("question_number") %>"><span class="ada-hide"><%=questions(q).Item("label") %></span><%=questions(q).Item("question") %></label></div></td>
<%
					If two_bank Then
						If (q+lo_op) <= UBound(questions) Then
							...
							<td width="50%" style="height: 30px; padding: 8px 0px 2px 9px;" valign="top"><div style="margin: 0px;"><label class="bac" for="q-<%=questions(q+lo_op).Item("question_number") %>"><span class="ada-hide"><%=questions(q+lo_op).Item("label") %></span><%=questions(q+lo_op).Item("question") %></label></div></td>
<%
						Else %>
