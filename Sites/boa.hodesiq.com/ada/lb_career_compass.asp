...
'*** WHICH PAGE TO SHOW AND WHICH PAGE TO STORE ***'
	...
	If ExistsInRequest("restart.x") Or request_page_number = 0 Then
		session.Contents.Remove("lbpages")
...
'******** CREATE NEW PAGES ********'
		...
		session.Contents("lbpages") = pages
	End If
'*** WHICH PAGE TO SHOW AND WHICH PAGE TO STORE ***'
	
'************** STORE PAGE ANSWERS ****************'
	If store_page_number > 0 And store_page_number < 4 Then
		If isArray(session.Contents("lbpages")) Then
			store_pages = session.Contents("lbpages")
			...
			session.Contents("lbpages") = store_pages
		End If'If isArray(session.Contents("lbpages")) Then
	End If
...
'********** REDIRECT TO APPROPRIATE PAGE **********'
	select case request_page_number
		case 1
			Response.Redirect("lb_career_compass_1.asp")
		case 2
			Response.Redirect("lb_career_compass_2.asp")
		case 3
			Response.Redirect("lb_career_compass_3.asp")
		case 4
			Response.Redirect("lb_career_compass_4.asp")
		case else
			Response.Redirect("lb_career_compass_1.asp")
	end select
